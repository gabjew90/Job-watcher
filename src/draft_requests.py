...
# Written by the pipeline, read by `python -m src.deliver` after the push.
DELIVERY_FILE = Path("state/pending_delivery.json")
...
def process(raw_jobs: list[Job], seen: dict, config: dict) -> list:
    ...
    for issue in resp.json():
        ...
        # Announced only once the files are pushed — see queue_delivery.
        queue_delivery(number, _format_comment(result, repo, branch), close=True)
        log.info("Drafted resume: %s (delivery queued)", result.docx_path)
    return written
...
def _check(resp: requests.Response, what: str, number: int) -> bool:
    """These calls used to be fire-and-forget, so a failed close looked
    exactly like a successful one."""
    if resp.status_code >= 400:
        log.warning("GitHub %s on issue #%d failed: %s %s",
                    what, number, resp.status_code, resp.text[:200])
        return False
    return True


def _comment(token: str, repo: str, number: int, text: str) -> bool:
    body = f"{text}\n\n---\n_Generated by [Claude Code](https://claude.ai/code)_"
    return _check(requests.post(f"{API}/repos/{repo}/issues/{number}/comments",
                                json={"body": body}, headers=_gh(token), timeout=30),
                  "comment", number)


def _close(token: str, repo: str, number: int) -> bool:
    return _check(requests.patch(f"{API}/repos/{repo}/issues/{number}",
                                 json={"state": "closed"}, headers=_gh(token), timeout=30),
                  "close", number)


def _label(token: str, repo: str, number: int, label: str) -> bool:
    return _check(requests.post(f"{API}/repos/{repo}/issues/{number}/labels",
                                json={"labels": [label]}, headers=_gh(token), timeout=30),
                  "label", number)


def queue_delivery(number: int, comment: str, close: bool = False) -> None:
    """Hold an announcement until the files it links to are pushed.

    The comment links to files in the repo and closing the issue ends the
    request. Both used to happen inside the pipeline, before the workflow's
    commit-and-push step, so a failed push left the request closed pointing
    at files that were never saved. The workflow runs `python -m src.deliver`
    after the push instead; if the push fails the queue is never read and
    the issue stays open for the next run.
    """
    queue = load_queue()
    queue.append({"number": number, "comment": comment, "close": close})
    DELIVERY_FILE.parent.mkdir(parents=True, exist_ok=True)
    DELIVERY_FILE.write_text(json.dumps(queue, indent=1) + "\n")


def load_queue() -> list[dict]:
    if DELIVERY_FILE.exists():
        return json.loads(DELIVERY_FILE.read_text())
    return []
...
def deliver_queued() -> int:
    """Post everything the pipeline queued. Returns the number delivered."""
    token = os.environ.get("GITHUB_TOKEN")
    repo = os.environ.get("GITHUB_REPOSITORY")
    queue = load_queue()
    if not queue:
    ...
    if not (token and repo):
        log.warning("No GITHUB_TOKEN; %d queued delivery(ies) left in place", len(queue))
        return 0
    delivered, kept = 0, []
    for item in queue:
        number = item["number"]
        if not _comment(token, repo, number, item["comment"]):
            kept.append(item)  # retried next run; the issue is still open
            continue
        if item.get("close"):
            _close(token, repo, number)
        delivered += 1
        log.info("Delivered draft for issue #%d", number)
    if kept:
        DELIVERY_FILE.write_text(json.dumps(kept, indent=1) + "\n")
    else:
        clear_queue()
    return delivered
