...
- **Expanded product portfolio from 4-hour to 2-hour duration** with an AC-integrated containerized BESS to capture emerging AI data center demand; secured executive and commercial buy-in to establish the platform as LG's flagship offering, driving multi-GWh forward bookings for 2027 delivery.
- **Built bottom-up cell-to-system cost, EPC installation, and logistics models** to set competitive system pricing, evaluate PCS configurations, and quantify the margin, energy density, and transport advantages of the 2-hour architecture.
- **Translated hyperscale and AI data center power constraints** into core BESS requirements: authored specifications for compute step-load transient smoothing, voltage fault ride-through with active recovery, gas turbine pairing, black start and islanding, and capacity management for non-firm interconnection with curtailment.
- **Led PCS vendor selection and owned the UL 9540 listing path**; served as the product interface between engineering, sales, and project execution, aligning volume commitments, cost-down trajectory, development/certification schedules, technical documentation, and commissioning procedures.

**Senior Systems Engineer, Grid-Scale BESS (LG Chem)** *(Feb 2018 – Feb 2021)*
- **Led product engineering for LG's first prefabricated containerized BESS** (2.86 MWh/unit); partnered with contract manufacturer Jabil from architecture through functional prototype, managing cost, safety, and size/weight tradeoffs to replace bespoke field assembly and deliver nearly 1 GWh across CAISO and ERCOT.
...
- **Engineered an automated pipeline ingesting 100+ research PDFs daily** (16,000+ total); parsed unstructured multi-asset commentary into structured JSON schemas with verbatim anchor quotes, programmatically verifying every extracted metric against source text before database commit.
- **Built the automated “Morning Pulse” briefing, published every trading day** (110+ issues) with a lead theme, desk-call trade board, and economic and earnings calendar drawn from live options chains and agency APIs; a redesigned synthesis pipeline in shadow pilot raised extraction fidelity against source PDFs from 33–60% to 87–100% and cut grouping fragmentation from 30–60% to 9–16%.
- **Built an interactive Q&A agent executing cited answers over the research corpus** (1,470+ production queries); regex-based tool router restricts model actions per query, and a payload provenance guard blocks any figure absent from tool output.
